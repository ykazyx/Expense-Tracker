...
def home_view(request):
    ...
    if request.method == 'POST':
        ...
        if len(sale_qs) > 0:
            ...
            sales_df.rename({'customer_id': 'Shopkeeper', 'salesman_id': 'Person', 'id': 'sales_id'}, axis=1, inplace=True)
            positions_data = []
            for sale in sale_qs:
...
def sale_list_view(request):
    qs = Sale.objects.all()
    return render(request, 'transactions/main.html', {'qs':qs})


def sale_detail_view(request, **kwargs):
    pk = kwargs.get('pk')
    obj = Sale.objects.get(pk=pk)
    return render(request, 'transactions/main.html', {'object':pk})


'''
in the urls:
path('transactions/', sale_list_view, name='list'),
path('transactions/<pk>/', sale_detail_view, name='detail'),
'''